...
public class FinalDb {
	 ...
	 * 删除所有数据表
	 */
	public void dropDb(SQLiteDatabase tempDb) {

		if (tempDb == null) {
			tempDb = db;
		}
		Cursor cursor = tempDb
				.rawQuery(
						"SELECT name FROM sqlite_master WHERE type ='table' AND name != 'sqlite_sequence'",
						null);
		if (cursor != null) {
			while (cursor.moveToNext()) {
				tempDb.execSQL("DROP TABLE " + cursor.getString(0));
			}
		}
		if (cursor != null) {
			cursor.close();
	...
	public void exeCustomerSql(String sql) {
		db.execSQL(sql);
	 ...
	 * @return
	 */
	public <T> T findUniqueByWhere(Class<T> clazz, String strWhere) {
		checkTableExist(clazz);
		return findUniqueBySql(clazz,
				SqlBuilder.getSelectSQLByWhere(clazz, strWhere));
	}
	 ...
	 * @return
	 */
	public <T> T findUniqueBySql(Class<T> clazz, String strSQL) {
		checkTableExist(clazz);
		debugSql(strSQL);
		Cursor cursor = db.rawQuery(strSQL, null);
		try {
	...
	/**
	 * 根据SQL语句查询所有数据并返回Cursor类型
	 * 
	 * @param clazz
	 * @param strSQL
	 * @return
	 */
	public Cursor findAllBySqlReturnCursor(Class clazz, String strSQL) {
		checkTableExist(clazz);
		debugSql(strSQL);
		Cursor cursor = db.rawQuery(strSQL, null);
		try {
